...
from score_extraction import extract_scores  # Import new function
...
if uploaded_file is not None:
    ...
    if all(acc >= 50 for acc in [family_name_accuracy, first_name_accuracy, student_number_accuracy]):
        ...
        if valid_scores and results_db is not None:
            ...
            st.warning("⚠️ Scores are not valid. Data will not be added to the database.")
    else:
        st.warning("⚠️ Low accuracy detected, Skipping score extraction and moving for manual extraction.")
